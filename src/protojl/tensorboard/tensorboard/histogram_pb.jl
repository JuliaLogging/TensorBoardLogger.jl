...
# original file: tensorboard/compat/proto/histogram.proto (proto3 syntax)
...
function PB.decode(d::PB.AbstractProtoDecoder, ::Type{<:HistogramProto}, _endpos::Int=0, _group::Bool=false)
    min = zero(Float64)
    max = zero(Float64)
    num = zero(Float64)
    sum = zero(Float64)
